...
async function getHandler(
  ...
  const { session } = authResult
    ...
    const response = await fetch(`${config.baseUrl}/api/alertmanager/grafana/api/v2/alerts`, {
      method: 'GET',
      headers,
    })
    ...
    if (!response.ok) {
      ...
      const altResponse = await fetch(`${config.baseUrl}/api/alerts`, {
        method: 'GET',
        headers,
      })
      if (!altResponse.ok) {
      ...
      const alerts = await altResponse.json()
...
export const GET = withErrorHandling(getHandler, 'GET /api/grafana/[instanceId]/alerts')
